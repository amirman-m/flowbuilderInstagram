// GeminiChatNode Executor - SOLID-compliant orchestration for Google Gemini chat nodes
...
/**
 * Specialized executor for Google Gemini Chat nodes
 * Handles AI input processing and response normalization
 */
export class GeminiChatNodeExecutor extends NodeExecutor {
  ...
  /**
   * Execute Gemini node with message input (from ChatInput or other nodes)
   */
  ...
  protected async validateInputs(inputs: Record<string, any>): Promise<void> {
    ...
    if (!inputs.message_data && !inputs.user_input) {
      throw new Error('Gemini node requires message_data or user_input');
    }

    const settings = this.getCurrentSettings();
    if (!settings.model) {
      throw new Error('Gemini node is not configured: please open Settings and select a Model (e.g., gemini-1.5-flash). Tip: double-click the Gemini node to open its settings.');
    }
  }
